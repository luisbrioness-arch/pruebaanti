// Envoltorio sobre html5-qrcode (vendorizada en js/vendor/, ver
// docs/tecnico-app.md). Abre un modal de cámara de pantalla completa y
// resuelve con la serie leída — por cámara o por el campo de entrada manual
// que siempre queda visible al lado (ver respuestas del diseño original:
// el escaneo nunca puede ser la única puerta de entrada, la cámara falla,
// el código viene rayado, etc.).
...
export function abrirScanner() {
  return new Promise((resolve) => {
    const overlay = el(`
      <div class="scanner-modal">
        <div class="scanner-header">
          ...
          <button type="button" class="topbar-boton" id="scanner-cerrar" aria-label="Cerrar">✕</button>
        </div>
        ...
        <div class="scanner-footer">
          ...
          <div class="entrada-manual">
            <input type="text" id="scanner-manual-input" placeholder="O escribe la serie a mano" autocomplete="off" autocapitalize="characters">
    ...
    $manualInput.addEventListener('keydown', (ev) => {
      if (ev.key === 'Enter') overlay.querySelector('#scanner-manual-btn').click();
    });
    ...
    if (lector) {
      lector
        ...
        .catch(() => {
          $mensaje.textContent = 'No se pudo abrir la cámara (¿permiso denegado?) — escribe la serie a mano.';
        });
    }
  });
}
