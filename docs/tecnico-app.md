...
**Ya no se precarga ningún kit estándar** (mejora pedida: "eliminar stock básico de cada trabajo y tener un buscador de ítem y agregar cantidad") — la orden nace con la lista de ferretería vacía. El técnico busca lo que usó escribiendo en un campo de texto (filtra `GET /api/catalogo/items-ferreteria`, catálogo completo cacheado en `storage.js` desde Inicio — ver `setCatalogoFerreteria`/`getCatalogoFerreteria`), toca el resultado para agregarlo con una cantidad por defecto, y ajusta esa cantidad con el mismo stepper de siempre (o lo quita con 🗑️). El backend (`OrdenWizardService::resolverItemsFerreteria`) sigue aceptando cualquier `item_ferreteria_id` válido — nunca dependió de que viniera de un kit. **Actualización:** la tabla `kits_servicio_item` y el editor de "Kits estándar" del panel admin ya no existen (pedido: *"eliminar el kit standar de todo el proyecto que no exista"*) — no era solo que el wizard no los usara, era código y schema sin ningún consumidor real. Ver `docs/admin-api.md`.
...
| Ferretería / cierre técnico | Sí | Ya no hay nada que precargar (no se usa ningún kit — ver arriba); el buscador de ítems depende del catálogo cacheado desde Inicio (`getCatalogoFerreteria`), que ya se pedía de todas formas. Sin ese catálogo en caché, el técnico sigue pudiendo cerrar la orden sin ferretería. |
