...
**Pendiente** (no resuelto en este cambio): hoy un técnico solo puede enlazar una orden a una venta que él mismo registró (`ForbiddenException` si `vendedor_id` no coincide) — instalar la venta de otro técnico, o una venta cargada directo por TuVes sin vendedor interno, no es posible todavía. Si se necesita ese flujo, hace falta además: (1) abrir la selección de venta más allá de "las mías" en el paso 1 del wizard y en el registro retroactivo, y (2) para "venta directa de TuVes" sin técnico vendedor, decidir cómo se representa (`vendedor_id` nulo, o un usuario especial "TuVes") — `ventas.vendedor_id` hoy es `NOT NULL`.
