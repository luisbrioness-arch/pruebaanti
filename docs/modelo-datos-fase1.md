...
El paso 5 del wizard de instalación (cuando `orden.venta_id` no es nulo) hace dos snapshots en el mismo instante: el de la orden (`monto_bruto`, `monto_tecnico`, con el `porcentaje_reparto` del TÉCNICO que instala) y el de la venta (`monto_comision`, `monto_vendedor`, con el `porcentaje_reparto` de `venta.vendedor_id` — quien la REGISTRÓ, no necesariamente el mismo usuario). Es deliberado que sea un solo momento — evita que exista un estado intermedio donde la orden ya diga "enviada" pero la venta todavía no sepa que se instaló.

**FIX aplicado** (pedido: *"si la venta viene de otro lugar ya sea directa de tuvez o otro tecnico esa no se paga al que instala si no al que vendio"*) — `OrdenWizardService::confirmarVenta()` acreditaba la comisión al TÉCNICO que instala, no a `venta.vendedor_id`. Hoy no era observable (`crearOReanudarBorrador`/`crearRetroactiva` solo dejan enlazar una venta si `vendedor_id === tecnico_id` que instala — ver "Pendiente" abajo), pero era un bug latente listo para activarse en cuanto esa restricción se abra.

**Resuelto** (pedido: "todos", sobre la lista de pendientes de este documento): las dos restricciones de arriba ya no existen.
1. `OrdenWizardService::crearOReanudarBorrador()` y `crearRetroactiva()` ya no exigen `venta.vendedor_id === tecnicoId` — cualquier técnico puede enlazar cualquier venta `registrada`, propia o ajena. `VentaController::pendientes()`/`detalle()` dejaron de estar restringidas al dueño (antes 403).
2. `ventas.vendedor_id` pasó a ser `NULL`able — el formulario "Registrar venta" del técnico tiene una casilla ("Venta directa de TuVes — yo no la vendí") que la registra sin vendedor. `confirmarVenta()` detecta `vendedor_id === null` y la pasa a `instalada` sin calcular comisión (`monto_comision`/`monto_vendedor` quedan `NULL`) — nadie cobra por una venta que nadie vendió internamente. Los `JOIN usuarios` sobre `ventas.vendedor_id` en `VentaRepository` (pendientesInstalarTodas, historial) pasaron a `LEFT JOIN` para no ocultar estas ventas; el frontend las muestra como "TuVes (directo)".
